...
// Takes all the available vehicles on that location inside the reservation slot, and gets the model names for further use
  const loadAvailableModels = () => {
    ...
    ModelService.fetchAvailableVehicles(selectedPlace.id,dateStart,dateEnd,timeStart,timeEnd).then((data) => {
        let allVehicles = data.data;
        const duplicateModels = allVehicles.map(vehicle => vehicle.model.modelName);
        //   {
  ...
  useEffect(() => {
    ...
    //   if(endDiff > 0){
        ...
        Object.keys(selectedPlace).length > 0 ? loadAvailableModels()
  ...
  return (
    <Fragment>
      <Grid container direction="column" alignItems="center" justify="space-around">
        ...
        <FormFields vehicles={availableModels} promotions={promotions} dateStart={selectedStartDate} dateEnd={selectedEndDate} location={selectedPlace}/>
          </Paper>
        </Grid>
      </Grid>
    </Fragment>
