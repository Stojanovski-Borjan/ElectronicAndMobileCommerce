...
import './Form.css'
import {FormControl, Grid,OutlinedInput, Select,InputLabel,MenuItem,makeStyles,useTheme,TextField,Button,Dialog,DialogActions,DialogContent,DialogContentText,DialogTitle,Slide, IconButton, InputAdornment, Icon} from '@material-ui/core';
...
const MyNumberInput = ({id,InputProps, ...props}) => {
  ...
  return (
    <TextField {...field} type="number" id={id} label="Број" variant="outlined" InputProps={InputProps} InputLabelProps={{shrink: true}}  error={!!errorText} />
  )
};
...
function getStyles(name, vehicleName, theme) {
  return {
    fontWeight:
      vehicleName.indexOf(name) === -1 ? theme.typography.fontWeightRegular
...
const myValidation = (amount,maxAmount,initMax,index) => {
  let errMsg;
  if(index !== 0){
  if(amount > maxAmount) errMsg = "За жал нема повеќе слободни возила од овој модел. Изберете друг модел на возило!";
  }
  else {
    if(amount > initMax ) errMsg = "За жал нема повеќе слободни возила од овој модел. Изберете друг модел на возило!";
  }
  return errMsg;
}
...
const RentSchema = Yup.object().shape({
  ...
  phone: Yup.number()
    ...
    .required('Задолжително!'),
  models: Yup.array().of(Yup.object({
      name: Yup.string()
      .required('Мора да изберете модел на возило!'),
      amount: Yup.number()
      .required("Изберете едно или повеќе возила!")
      .min(1,"Изберете едно или повеќе возила!")
      .max(50,"За жал нема повеќе слободни возила од овој модел. Изберете друг модел на возило!")
    }))
});
export default function FormFields({dateStart,dateEnd,location,vehicles,promotions}){
    ...
    useEffect(() => {
      ...
      if(Object.keys(location).length > 0){
        ...
        momentPrice = models.map(model => {
          return location.vehicles.find(veh => veh.model.modelName === model.name).model.pricePerMinute * model.amount
        }).reduce((a,b) => a + b,0);
  ...
  const handleClose = () => {
    ...
    return (
      ...
      initialValues={{
        ...
        models: [ { name: vehicles[0].name, amount: 1, maxAmount: vehicles[0].amount}],
        promotion: "None"
      }}
    ...
    >
      {({ errors, touched, isSubmitting, values, handleChange}) => { 
        ...
        values.promotion === "None" ? setDiscount(0) : setDiscount(promotions.find(p => p.name === values.promotion).discount);
        return (
        <Form>
          ...
          <FieldArray name="models" >
            { arrayHelpers => (
              <Grid item container xs={12} direction="column" alignItems="center">
                <Grid item>
                <Button className={classes.button} variant="outlined" size="medium" color="primary" onClick={() => {
                  const uniqueModels = vehicles.filter(m => values.models.filter(m1 => m1.name === m.name).length ===0);
                ...
                { 
                  ...
                  return (
                      <Grid item container key={index} justify="center" alignItems="center">
                        <Field name={`models.${index}.name`} className={classes.margin} type="select" as={Select} onChange={(e) => {handleChange(e); model.maxAmount = vehicles.find(vh => vh.name === model.name ).maxAmount ;}}> {/* Will Formik rerender after a change happens here ? It should happen. */}
                        { // Show only options(VHmodels) that are not already choosen
                          vehicles && vehicles.filter(opt => values.models
                          .filter(ml => model.name === ml.name ? false : ml.name === opt.name).length === 0).map( vh => (
                          <MenuItem key={vh.name} value={vh.name} style={getStyles(vh.name, vehicles, theme)}>
                            {vh.name}
                          </MenuItem>
                          ))
                        }
                        </Field>
                        <MyNumberInput name={`models.${index}.amount`} validate={() => myValidation(model.amount,model.maxAmount,vehicles[0].amount,index)}  id={`standard-number-${index}`} InputProps={ { inputProps: { "min": "1","max": `${model.maxAmount}`}}} />
                      
                        <IconButton area-label="delete" className={classes.margin} onClick={() => {
          ...
          { errors.models ? (
            <Grid item>
              <span className='validator'>{errors.models.find(m => m && (m.amount !== undefined || m.amount !== null)).amount}</span>
            </Grid>
            ) : null
          }
          <Grid item>
          <Field name="promotion" className={classes.margin} type="select" as={Select} >
            <MenuItem key="None" value="None">Промоција</MenuItem>
            {
              promotions && promotions.map( promo => (
                promo.name === "Regular" ? null : <MenuItem key={promo.name} value={promo.name} style={getStyles(promo.name, promotions.map(p => p.name), theme)}>{promo.name}</MenuItem>)
                )
              }
          </Field>
          </Grid>
          ...
          <Grid item>
            ...
            <OutlinedInput id="outline-amount" value={price}
            startAdornment={<InputAdornment position="start">ден</InputAdornment>}
            labelWidth={40}
            />
            </FormControl>
          </Grid>
          ...
          <Field className='form-field' name="email" validate={() => myEmailValidation(values.email)} type="email" placeholder="Електронска пошта"/>
          ...
          <Button type="submit" variant="contained" color="primary" className={classes.button} startIcon={<Icon>send</Icon>} disabled={isSubmitting}>РЕЗЕРВИРАЈ</Button>
